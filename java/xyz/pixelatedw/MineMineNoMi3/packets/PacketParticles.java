		...
		public IMessage onMessage(PacketParticles message, MessageContext ctx) 
		{
			...
			if(message.fx.equals("daienkai"))
			{
				for (int i = 0; i < 20; i++)
				{
					...
					player.worldObj.spawnParticle(EnumParticleTypes.FLAME.getParticleName(), message.posX + offsetX, message.posY + offsetY, message.posZ + offsetZ, 0.0D, 0.1D, 0.0D);
				}		
			}
			if(message.fx.equals("daienkai2"))
			{
				Timer timer = new Timer(true); 
				timer.schedule(ListParticleEffects.createChargeEffect1FX(player, message.posX, message.posY + 0.5, message.posZ, EnumParticleTypes.FLAME.getParticleName(), 1.0, 1, 0.45, 0.2), 0);	
			}
			if(message.fx.equals("fubuki"))
			{
				for (int i = 0; i < 1024 * 15; i++)
				{
					...
					player.worldObj.spawnParticle(EnumParticleTypes.SNOW_SHOVEL.getParticleName(), message.posX + offsetX, (message.posY + offsetY) + 3, message.posZ + offsetZ, 0.0D, 0.0D, 0.0D);
				}		
			}
			if(message.fx.equals("whiteLauncher"))
			{
				for (int i = 0; i < 20; i++)
				{
					...
					player.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL.getParticleName(), message.posX + offsetX, message.posY + offsetY, message.posZ + offsetZ, 0.0D, 0.1D, 0.0D);
				}	
			}
			...
			{
				Timer timer = new Timer(true); 
				timer.schedule(ListParticleEffects.createTornadoFX(player, message.posX, message.posY, message.posZ, "custom_suna", 2.0, 1, 0.35, 0.7), 0);
			}
			if(message.fx.equals("groundDeath"))
			{
				Timer timer = new Timer(true); 
				timer.schedule(ListParticleEffects.createChargeEffect1FX(player, message.posX, message.posY, message.posZ, "custom_suna", 0.5, 1, 0.80, 0.2), 0);
			}
