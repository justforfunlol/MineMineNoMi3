...
public class ModelPhoenixFull extends ModelZoanMorph
{
	...
	{		
		...
		{
			int cycleIndexFlyWing = (int) ((ent.ticksExisted * 5) % (wingAnimationArray.length - 1));

			LeftArm1.rotateAngleZ = (float) degToRad(wingAnimationArray[cycleIndexFlyWing]);
			RightArm1.rotateAngleZ = (float) -degToRad(wingAnimationArray[cycleIndexFlyWing]);
			
			LeftWing2.rotateAngleZ = (float) degToRad(wingAnimationArray[cycleIndexFlyWing]);
			RightWing2.rotateAngleZ = (float) -degToRad(wingAnimationArray[cycleIndexFlyWing]);
			
			int cycleIndexTail = (int) ((ent.ticksExisted * 2) % (tailAnimationArray.length - 1));
			int cycleIndexTail2 = (int) ((ent.ticksExisted * 3) % (tailAnimationArray.length - 1));

			Tails1.rotateAngleX = (float) degToRad(tailAnimationArray[cycleIndexTail]);
			Tails2.rotateAngleX = (float) -degToRad(tailAnimationArray[cycleIndexTail]);
			
			Tails11.rotateAngleX = (float) -degToRad(tailAnimationArray[cycleIndexTail2]);
			Tails22.rotateAngleX = (float) degToRad(tailAnimationArray[cycleIndexTail2]);
		}
	}
