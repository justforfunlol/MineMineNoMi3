...
public class AbilityItem extends Item
{
    ...
    public ActionResult<ItemStack> onItemRightClick(ItemStack itemStack, World world, EntityPlayer player, EnumHand hand)
    {
		...
		{		
			...
			//Minecraft.getMinecraft().effectRenderer.addEffect(new MainParticle(world, player.posX, player.posY, player.posZ, 5, 80));
			...
			{	
				...
				if(!this.attr.isFalseAttribute())
				{
					...
					{
						AbilityProjectile proj = new AbilityProjectile(world, player, attr);
						proj.setHeadingFromThrower(player, player.rotationPitch, player.rotationYaw, 0, 1.5F, 1);
						world.spawnEntityInWorld(proj);
					}
					
					if(this.attr.hasEffectOnUser())
					...
					if(this.attr.hasAOE()) 
						for(PotionEffect p : this.attr.getPotionEffectList())
							for(EntityLivingBase l : WyPI.Utils.getEntitiesNear(player, this.attr.getAOERadius()))
								l.addPotionEffect(new PotionEffect(p));
	
					if(this.attr.isPlayerSourceOfExplosion() && !this.attr.canItemBeCharged())
						world.newExplosion(player, player.posX, player.posY, player.posZ, this.attr.getExplosion(), this.attr.hasFire(), this.attr.hasExplosion());
				}
	...
	{
		if (itemStack.getTagCompound() != null && this.attr != null) 
		{			
